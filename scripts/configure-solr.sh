...
#####
#####  This script only for DrupalVM(Vagrant)
#####  Use it as step-by-step for any other env.
#####
...
SOLR_DOWNLOAD="https://ftp.drupal.org/files/projects/search_api_solr-8.x-2.x-dev.tar.gz"
...
if [ ! -e "$SOLR_SETUP_COMPLETE_FILE" ]; then
  ...
  sudo su - solr -c "/opt/solr/bin/solr create -c $SOLR_CORE_NAME"
  sudo su - solr -c "cp -r $SOLR_DOWNLOAD_DIR/$SOLR_MODULE_NAME/solr-conf/$SOLR_VERSION/* $SOLR_CORE_PATH/conf"
